...
class CIPairWiseEnvUsingDiff(gym.Env):
    def __init__(self, cycle_logs: CICycleLog, conf: Config):
        super(CIPairWiseEnvUsingDiff, self).__init__()
        self.conf = conf
        ...
        self.observation_space = spaces.Box(low=-1, high=1, shape=(self.testcase_vector_size,))  # ID, execution time and LastResults

        data = []
        tc_ids = []
        for tc in self.cycle_logs.test_cases:
            data.append(cycle_logs.export_test_case(tc, 
                option="list_avg_exec_with_failed_history", 
                pad_digit=-1, 
                win_size=self.conf.win_size))
            tc_ids.append(tc["test_id"])

        X = np.array(data)
        X = preprocessing.MinMaxScaler().fit_transform(X)
        self.observations = {tc_id: X[index]  for index, tc_id in enumerate(tc_ids)}
    ...
    def get_pair_data(self, current_indexes):
        [tc1, tc2] = [self.test_cases_vector[index]["test_id"] for index in current_indexes]
        return self.observations[tc1] - self.observations[tc2]
