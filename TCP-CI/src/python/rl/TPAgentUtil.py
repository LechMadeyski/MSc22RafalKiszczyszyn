...
class TPAgentUtil:
    ...
    def create_model(algo, env, params):
        ...
        if algo.upper() == "DQN":
            ...
            if params["policy"] == "MLP":
                model = ACER(MlpPolicy, env, replay_ratio=params["replay_ratio"], verbose=0)
            elif params["policy"] == "LSTM":
                model = ACER(MlpLnLstmPolicy, env, replay_ratio=params["replay_ratio"], verbose=0)
